...
*NET PAYABLE: ${currency}${line.net_payable}*`;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-900/40 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white border border-stone-200 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative text-stone-900 animate-in fade-in zoom-in-95 print:p-0 print:border-none print:shadow-none print:bg-white print:text-black">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-stone-500 hover:text-stone-900 p-1 rounded-lg hover:bg-stone-100 print:hidden"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="text-center border-b border-stone-200 pb-4 mb-4">
          <div className="flex justify-center items-center space-x-2">
            ...
            <h2 className="text-xl font-black text-stone-900">{settings?.factory_name || 'Garment Factory'}</h2>
          </div>
          <p className="text-xs text-stone-600 mt-1">Official Piece-Rate Worker Payslip</p>
          <div className="text-xs font-mono text-amber-900 mt-2 bg-amber-50 inline-block px-3 py-1 rounded-full border border-amber-200">
            Period: {period.start_date} to {period.end_date}
          </div>
        </div>

        {/* Worker Info */}
        <div className="flex items-center space-x-4 bg-stone-50 p-3 rounded-xl mb-4 border border-stone-200">
          <WorkerAvatar
          ...
          />
          <div>
            <div className="font-bold text-base text-stone-900">{worker?.full_name}</div>
            <div className="text-xs text-stone-600 font-mono">
              Code: {worker?.worker_code} • Line: {worker?.line_no} • Section: {worker?.section}
            </div>
            <div className="text-[11px] text-indigo-700 font-mono capitalize">
        ...
        <div className="space-y-2 text-xs font-mono mb-6">
          ...
          <div className="flex justify-between py-1 border-b border-stone-200">
            <span className="text-stone-600">Overtime Allowance:</span>
            <span className="text-stone-900">{currency}{(line.ot_amount || 0).toFixed(2)}</span>
          </div>

          <div className="flex justify-between py-1 border-b border-stone-200">
            <span className="text-stone-600">Bonus & Allowances:</span>
            <span className="text-stone-900">{currency}{((line.bonus_amount || 0) + (line.allowance_amount || 0)).toFixed(2)}</span>
          </div>
        ...
        <div className="flex gap-2 print:hidden">
          <button
            ...
            className="flex-1 flex items-center justify-center space-x-2 bg-emerald-700 hover:bg-emerald-800 text-white font-bold py-2.5 rounded-xl text-xs"
          >
          ...
          <button
            onClick={handlePrint}
            className="flex-1 flex items-center justify-center space-x-2 bg-stone-100 hover:bg-stone-200 text-stone-800 font-bold py-2.5 rounded-xl text-xs border border-stone-200"
          >
